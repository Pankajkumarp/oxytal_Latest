 ...
 * no Contentful wiring, keeps the reference's own colour identity
 * (`--ink` `#1E140C`, `--accent` `#A8682A`, `--accent-2` `#D9A05B`, the
 * `--deep-1`/`--deep-2` `#160D07`/`#2A1B10` warm-brown gradient) rather
 * than the site's per-page `themeColor` accent, and typography stays the
 * site's own inherited `Poppins` instead of the reference's Google Fonts
 * — tracking/size/weight/uppercase alone carry the reference's own mono/
 * display feel. Screenshots are real, already-hosted photography
 * (Contentful assets and Oxytal's S3 bucket, at the reference's own
 * URLs), so plain `<img>` tags rather than placeholder cards.
 *
  ...
  return (
    <nav aria-label="Breadcrumb" className="bg-[#160D07] py-4 pt-26">
      <div className="mx-auto max-w-7xl px-5 sm:px-8">
        <ol className="flex flex-wrap items-center gap-2 font-mono text-[12px] tracking-[0.09em] text-[#8A7562] uppercase">
          <li>
  ...
  return (
    ...
    >
      ...
      <div className="relative mx-auto max-w-7xl px-5 sm:px-8">
        <p ref={clientRef} className="mb-4.5 font-mono text-[12px] tracking-[0.18em] text-[#D9A05B] uppercase">
        ...
        >
          {FACTS.map((fact) => (
            <div key={fact.k} className="bg-[#160D07] px-5 py-4.5">
              <dt className="mb-1.5 font-mono text-[11px] tracking-[0.12em] text-[#8A7562] uppercase">{fact.k}</dt>
        ...
        >
          ...
          <img
            src="https://images.ctfassets.net/2ctrlpw4si8r/5rN8pXjYf81OJlJsQc7RyN/91074d680f5cac8db16497cacfaeb3c7/Deck-Slides-Bottle-Closeup.webp"
...
function BuiltRow({ row }: { row: (typeof BUILT_ROWS)[number] }) {
  ...
  return (
    <div className="grid grid-cols-1 items-center gap-8 lg:grid-cols-2 lg:gap-14">
      ...
      <div
        ref={mediaRef}
        className="overflow-hidden rounded-[18px] border border-[#EDE3D8] bg-[#F7F1E9] shadow-[0_20px_46px_-22px_rgba(30,20,12,0.24)]"
      >
        {/* eslint-disable-next-line @next/next/no-img-element -- matches the plain <img> convention this project already uses for external/hosted assets */}
        <img
          src={row.img}
...
function ConnectSection() {
  ...
  return (
    <section
      ...
      className="relative overflow-hidden bg-[#100A06] px-5 py-19 text-center sm:px-8 sm:py-24 lg:py-[150px]"
    >
      <div
        aria-hidden
        className="pointer-events-none absolute bottom-[-14%] left-1/2 h-[520px] w-[min(760px,90vw)] -translate-x-1/2 rounded-full"
        style={{ background: "radial-gradient(circle, rgba(217,160,91,.18), transparent 66%)" }}
      />

      <div className="relative">
        <span className="mb-0 inline-flex items-center gap-2.5 text-[11.5px] font-bold tracking-[0.22em] text-[#D9A05B] uppercase">
          Work with us
        </span>
        ...
        >
          Let&apos;s discuss what you&apos;re building — <em className="text-[#B8A48F] not-italic">and what&apos;s getting in the way.</em>
        </h2>
        ...
        >
          <span
            aria-hidden
            className="absolute -inset-3.5 animate-[spin_26s_linear_infinite] rounded-full border border-dashed border-white/14 transition-[border-color,opacity] duration-350 group-hover:border-[#D9A05B] group-hover:opacity-70"
          />
          <span
            aria-hidden
          ...
          />
          <span className="relative flex flex-col items-center gap-2.5 text-[16px] font-medium transition-colors duration-300 group-hover:text-[#100A06]">
            Let&apos;s connect
        ...
        <p className="mt-8.5 text-[14.5px] text-[#8A7562] sm:mt-12.5">
          Or email{" "}
          <a href="mailto:info@oxytal.com" className="border-b border-white/18 pb-0.5 text-[#B8A48F] transition-colors duration-150 hover:border-[#D9A05B] hover:text-white">
            info@oxytal.com
          </a>{" "}
...
export default function TaffersCaseStudy() {
  return (
