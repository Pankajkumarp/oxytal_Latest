 ...
 * (`--ink` `#1B0E17`, `--accent` `#D93A72`, `--accent-2` `#FF8FB8`, the
 * `--deep-1`/`--deep-2` `#150A12`/`#2C1224` plum gradient) rather than
 ...
 * "same size as the others" brief as `KaneffCaseStudy`) — 12px eyebrows,
  ...
  return (
    <nav aria-label="Breadcrumb" className="bg-[#150A12] py-4 pt-26">
      <div className="mx-auto max-w-7xl px-5 sm:px-8">
        <ol className="flex flex-wrap items-center gap-2 font-mono text-[12px] tracking-[0.09em] text-[#8C7182] uppercase">
          <li>
  ...
  return (
    ...
    >
      ...
      <div className="relative mx-auto max-w-7xl px-5 sm:px-8">
        <p ref={clientRef} className="mb-4.5 font-mono text-[12px] tracking-[0.18em] text-[#FF8FB8] uppercase">
        ...
        >
          {FACTS.map((fact) => (
            <div key={fact.k} className="bg-[#150A12] px-5 py-4.5">
              <dt className="mb-1.5 font-mono text-[11px] tracking-[0.12em] text-[#8C7182] uppercase">{fact.k}</dt>
        ...
        >
          ...
          <img
            src="https://images.ctfassets.net/twkb5au85wu1/66JymW7MmHlyFIuylcg6EZ/2c356f50b2a8ffc59f6cc13b9247c2a7/home-banner.webp"
        ...
        >
          {HARD_CARDS.map((card) => (
            <div key={card.n} className="bg-white p-7">
              <span className="mb-3.5 block font-mono text-[12px] tracking-[0.1em] text-[#D93A72]">{card.n}</span>
              <h3 className="mb-2.5 text-[19px] leading-[1.5] font-extrabold tracking-[-0.02em] text-[#1B0E17]">
                {card.title}
              </h3>
...
function BuiltRow({ row }: { row: (typeof BUILT_ROWS)[number] }) {
  ...
  return (
    <div className="grid grid-cols-1 items-center gap-8 lg:grid-cols-2 lg:gap-14">
      <div ref={textRef}>
        <span className="mb-3.5 block font-mono text-[12px] tracking-[0.11em] text-[#D93A72]">{row.n}</span>
        <h3 className="mb-3.5 text-[24px] leading-[1.4] font-extrabold tracking-[-0.02em] text-[#1B0E17]">
          {row.title}
        </h3>
      ...
      <div
        ref={mediaRef}
        className="overflow-hidden rounded-[18px] border border-[#F0E2E9] bg-[#FBF3F7] shadow-[0_20px_46px_-22px_rgba(27,14,23,0.22)]"
      >
        {/* eslint-disable-next-line @next/next/no-img-element -- matches the plain <img> convention this project already uses for external/hosted assets */}
        <img
          src={row.img}
      ...
      <div className="mx-auto max-w-5xl">
        ...
        <div ref={phasesRef} className="mt-9 overflow-hidden rounded-2xl border border-[#F0E2E9] bg-white">
          {PHASES.map((phase, index) => (
            ...
            >
              <span className="pt-1 font-mono text-[12px] tracking-[0.1em] text-[#D93A72]">{phase.n}</span>
              <div>
                <h3 className="mb-2 text-[18px] leading-[1.4] font-extrabold tracking-[-0.02em] text-[#1B0E17]">
                  {phase.title}
                </h3>
      ...
      <div className="mx-auto max-w-5xl">
        ...
        <div ref={gridRef} className="mt-8.5 grid grid-cols-1 gap-5 sm:grid-cols-2">
          {TECH_GROUPS.map((group) => (
            <div key={group.title}>
              <h4 className="mb-3 font-mono text-[16px] font-medium tracking-[0.12em] text-[#D93A72] uppercase">
                {group.title}
              </h4>
        ...
        <div ref={gridRef} className="mt-9 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {RELATED.map((item) => (
            ...
            >
              ...
              <div className="p-5.5">
                <span className="font-mono text-[12px] tracking-[0.11em] text-[#9C8A96] uppercase">{item.k}</span>
                <h3 className="mt-2 mb-1.5 text-[17px] font-extrabold text-[#1B0E17]">{item.title}</h3>
...
export default function CasaFamosaCaseStudy() {
  return (
