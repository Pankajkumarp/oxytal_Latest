...
// Contentful wiring — related cards only, same convention as
// `InkJetWorldCaseStudy`'s own. `entry.fields.elements` may hold up to 4
// `contentDetail` entries; `[0]` is reserved for this case study's own
// hero source elsewhere in the sibling files, but this `Hero` has no
// `.hero-shot` photo to override (see the doc comment below), so it's
// left untouched here. `[1]`–`[3]` each become one `RelatedSection` card
// (see `resolveRelatedItem`) — everything falls back to the hardcoded
// `RELATED` list individually when `entry` is omitted or those fields
// are unset.
 ...
 * colour identity (`--ink` `#0F1A10`, `--body` `#526155`, `--accent`
 * `#0B7A18`, `--accent-2` `#3B8632`, `--warm` `#C97A2B`, the
 ...
 * hardcoded default individually when unset. Unlike some siblings
 * there's no hero-photo or gallery override here — see below.
 *
 * Things unique to this one:
 * - Like `InkJetWorldCaseStudy`, the hero has no `.hero-shot` photo — the
 *   reference's own hero is text-and-facts only, and there's nothing
 *   here for Contentful to override.
  ...
  return (
    <nav aria-label="Breadcrumb" className="bg-[#08150A] py-4 pt-26">
      <div className="mx-auto max-w-7xl px-5 sm:px-8">
        <ol className="flex flex-wrap items-center gap-2 text-[12px] text-[#6E8470] uppercase">
          <li>
  ...
  return (
    ...
    >
      ...
      <div className="relative mx-auto max-w-7xl px-5 sm:px-8">
        <p ref={clientRef} className="mb-4.5 font-semibold text-[12px] text-[#3B8632] uppercase">
        ...
        >
          {FACTS.map((fact) => (
            <div key={fact.k} className="bg-[#08150A] px-5 py-4.5">
              <dt className="mb-1.5 font-semibold text-[11px] text-[#6E8470] uppercase">{fact.k}</dt>
        ...
        >
          {HARD_CARDS.map((card) => (
            <div key={card.n} className="bg-white p-7">
              <span className="mb-3.5 block font-semibold text-[12px] text-[#0B7A18]">{card.n}</span>
...
function BuiltRow({ row }: { row: (typeof BUILT_ROWS)[number] }) {
  ...
  return (
    <div className="grid grid-cols-1 items-center gap-8 lg:grid-cols-2 lg:gap-14">
      <div ref={textRef}>
        <span className="mb-3.5 block font-semibold text-[12px] text-[#0B7A18]">{row.n}</span>
      ...
      <div className="mx-auto max-w-5xl">
        ...
        <div ref={phasesRef} className="mt-9 overflow-hidden rounded-2xl border border-[#E3EBDF] bg-white">
          {PHASES.map((phase, index) => (
            ...
            >
              <span className="pt-1 font-semibold text-[12px] text-[#0B7A18]">{phase.n}</span>
              <div>
      ...
      <div className="mx-auto max-w-5xl">
        ...
        <div ref={gridRef} className="mt-8.5 grid grid-cols-1 gap-5 sm:grid-cols-2">
          {TECH_GROUPS.map((group) => (
            <div key={group.title}>
              <span className="mb-3 text-[16px] font-medium text-[#0B7A18] uppercase block">
                {group.title}
              </span>
        ...
        <div ref={gridRef} className="mt-9 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-2">
          {items.map((item) => (
            ...
            >
              ...
              <div className="p-5.5">
                <span className="font-semibold text-[12px] text-[#0B7A18] uppercase">{item.k}</span>
                <span className="mt-2 mb-1.5 text-[20px] font-extrabold text-[#0F1A10] block">{item.title}</span>
                <p className="text-[15px] leading-[1.75] text-[#526155]">{item.text}</p>
              </div>
            </Link>
          ))}
        </div>
...
export default function CastletownrocheCaseStudy({ entry }: Props) {
