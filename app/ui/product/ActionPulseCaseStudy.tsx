 ...
 * sibling `SamVaultCaseStudy`: this page's own body isn't wired to
 * Contentful, keeps the reference's own colour identity (`--navy`
 * `#0D0B1F`/`--indigo` `#4F46E5`/`--violet` `#7C3AED`/`--electric`
 * `#818CF8`) instead of the site's per-page `themeColor` accent, no
 * `<nav>`/`<footer>` (the app's global `Navbar`/footer already wrap every
 * page), and typography stays the site's own inherited `Poppins` rather
 * than the reference's own Google Fonts (`Syne`/`Inter`/`JetBrains
 * Mono`). `entry` (from `ComposableElementRenderer`'s `actionPulse`
 * registration) is read for exactly one thing: the "Explore other
 * products" section at the bottom (see `ExploreSection`/
 * `resolveRelatedItem` near the end of this file) — same narrow,
 * related-items-only Contentful usage `SamVaultCaseStudy`/
 * `ForgePipelineCaseStudy` each added for their own equivalent section.
 *
...
interface RelatedItem {
   ...
   * `SamVaultCaseStudy`/`ForgePipelineCaseStudy` each use.
   */
  tileBg?: string;
  alt: string;
  k: string;
 ...
 * `resolveRelatedItem` below) — the other products, since `ActionPulse`
 * is itself a product page (`/products/action-pulse`), not a client case
 * study. No `img` on any of these: there's no existing photographed
 * asset for the other 3 products anywhere in this codebase to link to
 * honestly, so each card shows a plain gradient tile (in that product's
 * own real brand colors, matching the identity each one's own file
 * establishes) with its name instead of guessing at an asset URL that
 * might 404.
 */
const RELATED_PRODUCTS: RelatedItem[] = [
  ...
  {
    href: "/products/kollabry",
    tileBg: "linear-gradient(135deg,#141127,#2C2459)",
    alt: "Kollabry",
    k: "Product · Delivery",
    title: "Kollabry",
    text: "Issues, sprints and retros in the same workspace as your knowledge base — track the work, capture the knowledge.",
  },
];
 ...
 * `ForgePipelineCaseStudy` each use.
 */
 ...
 * `ForgePipelineCaseStudy`/`TaffersCaseStudy` each use: `heroImage` for
 * the photo, `category` for the small tag, `title`/`shortDescription`
 * for the copy (capped at `RELATED_DESCRIPTION_MAX_LENGTH` via
 * `truncate`), and a link resolved from `cta` (preferred) or else
 * `/case-studies/<slug>`. Returns `undefined` for a missing entry or one
 * with no `heroImage` — a related card with no photo would look broken
 * here, so it's dropped rather than shown empty (the static
 * `RELATED_PRODUCTS` fallback handles the "no photo yet" case
 * differently, via `tileBg`, precisely because those are hand-authored
 * literals rather than editor-supplied CMS entries).
 */
...
function ExploreSection({ related }: { related?: RelatedItem[] }) {
  ...
  return (
    <section className="relative z-10 px-[5%] py-24">
      <div className="mx-auto max-w-[1200px]">
        <SectionHeader eyebrow="More of our work" title="Related case studies." headingLevel="h4" />
        <div ref={gridRef} className="grid grid-cols-1 gap-6 lg:grid-cols-2">
          {items.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="block overflow-hidden rounded-[20px] border border-[rgba(255,255,255,0.07)] bg-[rgba(255,255,255,0.04)] transition-all duration-200 hover:-translate-y-1.5 hover:border-[rgba(79,70,229,0.4)]"
            >
              {item.img ? (
              ...
              <div className="p-7">
                <span className="text-[12px] font-medium tracking-[0.1em] text-[#818CF8] uppercase">{item.k}</span>
                <span className="mt-2 mb-1.5 block text-[20px] font-bold text-white">{item.title}</span>
                <p className="text-[14px] leading-[1.65] text-[#A8A4CC]">{item.text}</p>
              </div>
            </Link>
          ))}
        </div>
...
export default function ActionPulseCaseStudy({ entry }: Props) {
  // Up to 3 related-product cards, one per `contentDetail` entry among
  // `entry.fields.elements` — same convention `SamVaultCaseStudy`/
  // `ForgePipelineCaseStudy` each use for their own equivalent section.
  // `resolveRelatedItem` drops any that don't resolve (missing entry, or
  // no `heroImage`), and `ExploreSection` falls back to
  // `RELATED_PRODUCTS` whenever none of them do.
