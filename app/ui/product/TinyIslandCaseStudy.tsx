  ...
  return (
    <nav aria-label="Breadcrumb" className="bg-[#061518] py-4 pt-26">
      <div className="mx-auto max-w-7xl px-5 sm:px-8">
        <ol className="flex flex-wrap items-center gap-2 text-[12px] text-[#6C8688] uppercase">
          <li>
  ...
  return (
    ...
    >
      ...
      <div className="relative mx-auto max-w-7xl px-5 sm:px-8">
        <p ref={clientRef} className="mb-4.5 font-semibold text-[12px] text-[#9C6A0D] uppercase">
        ...
        >
          {FACTS.map((fact) => (
            <div key={fact.k} className="bg-[#061518] px-5 py-4.5">
              <dt className="mb-1.5 font-semibold text-[11px]  text-[#6C8688] uppercase">{fact.k}</dt>
        ...
        >
          {HARD_CARDS.map((card) => (
            <div key={card.n} className="bg-white p-7">
              <span className="mb-3.5 block font-semibold text-[12px] text-[#9C6A0D]">{card.n}</span>
...
function BuiltRow({ row, image }: { row: (typeof BUILT_ROWS)[number]; image?: string }) {
  ...
  return (
    <div className="grid grid-cols-1 items-center gap-8 lg:grid-cols-2 lg:gap-14">
      <div ref={textRef}>
        <span className="mb-3.5 block font-semibold text-[12px] text-[#9C6A0D]">{row.n}</span>
      ...
      <div className="mx-auto max-w-5xl">
        ...
        <div ref={phasesRef} className="mt-9 overflow-hidden rounded-2xl border border-[#E1EBE9] bg-white">
          {PHASES.map((phase, index) => (
            ...
            >
              <span className="pt-1 font-semibold text-[12px] text-[#9C6A0D]">{phase.n}</span>
              <div>
      ...
      <div className="mx-auto max-w-5xl">
        ...
        <div ref={gridRef} className="mt-8.5 grid grid-cols-1 gap-5 sm:grid-cols-2">
          {TECH_GROUPS.map((group) => (
            <div key={group.title}>
              <span className="mb-3 font-semibold text-[16px] font-medium text-[#9C6A0D] uppercase block">
                {group.title}
              </span>
          ...
          {items.map((item) => (
            ...
            >
              ...
              <div className="p-5.5">
                ...
                <span className="mt-2 mb-1.5 text-[20px] font-extrabold text-[#0C1D20] block">{item.title}</span>
                <p className="text-[15px] leading-[1.75] text-[#4C6467]">{item.text}</p>
              </div>
            </Link>
          ))}
        </div>
