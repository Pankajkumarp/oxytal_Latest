...
function Hero({ backgroundUrl }: { backgroundUrl?: string }) {
  ...
  <div
    aria-hidden
    className="pointer-events-none relative z-[-1] w-full aspect-[1200/750] overflow-hidden lg:absolute lg:inset-y-0 lg:left-[49%] lg:right-0 lg:max-w-full lg:aspect-auto xl:left-[45%] 2xl:left-[42%]"
  >
    <img
      src={backgroundUrl}
      alt=""
    ...
    />
      ...
      <div className="container relative mx-auto px-5 md:px-10">
        <div ref={copyRef} className="relative z-10 max-w-full lg:max-w-[45%] xl:max-w-[42%] 2xl:max-w-[38%] pt-4 md:pt-6">
          ...
          <h1
            ref={titleRef}
            className="max-w-[680px] text-[28px] font-extrabold leading-[1.1] tracking-[-0.045em] sm:text-[34px] md:text-[40px] xl:text-[50px] mb-3 text-[#171122]"
          >
        ...
        >
          ...
          <ul className="flex flex-wrap gap-x-7 gap-y-4">
            {["Diageo", "Aviation Gin", "Casa Famosa", "Taffer's", "InkJet World"].map((name) => (
              <li key={name} className="text-[0.95rem] font-semibold text-[#656566]">
                {name}
              </li>
            ))}
          </ul>
...
// ───────────────────────── related + final CTA ─────────────────────────
const RELATED = [
  { k: "02", title: "Experience Design", body: "Testing the idea with people before building it.", href: "/services/ui-ux-design" },
  { k: "03", title: "Software Engineering", body: "Building what the plan decided was worth building.", href: "/services/software-development" },
  { k: "04", title: "Cloud & Transformation", body: "Moving off what the plan decided to retire.", href: "/services/cloud-digital-transformation" },
  { k: "07", title: "AI & Agentic Engineering", body: "Where automation genuinely pays back.", href: "/services/ai-and-intelligent-automation" },
];
